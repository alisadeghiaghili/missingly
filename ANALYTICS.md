...
- `/analysis/gee` fits generalized estimating equations for correlated observations. It provides population-averaged Gaussian, Binomial, and Poisson effects with robust sandwich standard errors, explicit treatment coding and selected interactions, plus Independence or Exchangeable working correlation. It requires at least three groups and rejects an Exchangeable fit when any complete group has fewer than two observations. GEE is not a random-effects model: its coefficients have a marginal, rather than subject-specific, interpretation. It does not yet provide autoregressive, nested, ordinal, multinomial, weighted, or small-cluster-corrected GEE.
...
- Models: population-averaged Gaussian, Binomial, and Poisson generalized estimating equations with robust sandwich standard errors and Independence or Exchangeable working correlation.
