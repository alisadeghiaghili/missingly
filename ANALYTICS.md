...
- `/analysis/gee` fits generalized estimating equations for correlated observations. It provides population-averaged Gaussian, Binomial, and Poisson effects with robust sandwich standard errors, explicit treatment coding and selected interactions, plus Independence, Exchangeable, or AR(1) working correlation. AR(1) requires a numeric time column with unique complete values within each group. It requires at least three groups and rejects Exchangeable or AR(1) fits when any complete group has fewer than two observations. GEE is not a random-effects model: its coefficients have a marginal, rather than subject-specific, interpretation. It does not yet provide nested, ordinal, multinomial, weighted, or small-cluster-corrected GEE.
