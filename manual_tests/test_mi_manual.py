...
def test_multiple_imputations(df):
    ...
    for i, imp_df in enumerate(imputations):
        ...
        print(f"Imputation {i + 1}: missing={missing}")
...
def test_scalar_pooling():
    ...
    estimates = [10.1, 10.4, 9.9, 10.3, 10.0]
    variances = [0.25, 0.22, 0.27, 0.21, 0.24]
...
def test_regression_pooling(imputations):
    ...
    for df in imputations:
        ...
        sigma2 = np.var(residuals, ddof=len(coef))
    ...
    np.save(f"{OUTPUT_DIR}/pooled_coef.npy", result["coef_"])
...
def main():
    ...
    test_scalar_pooling()
    test_regression_pooling(imputations)
